...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["inputexc"])) {
        $cou_name_err = "Enter updated Associate Name";
    }
...
if (!empty($_POST["inputexc"])) {
        ...
        $sql = "UPDATE associate SET name=? WHERE id=?";
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                logActivity('Update', 'Associate', 'Update Associate Name ' . $param_name .' by '.  $_SESSION["username"] , $_SESSION["id"] );    
    ...
    if(isset($_GET["uid"]) && !empty(trim($_GET["uid"]))){
        ...
        $sql = "SELECT * FROM associate WHERE id = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
                        ...
                        <div class="col-lg-6">
                            <div class="card shadow-lg bg-info text-white border-0 rounded-lg mt-5">
                                <div class="card-header">
                                    <h3 class="text-center font-weight-light text-info my-4">Update Associate Detail
                                    </h3>
                                </div>
                                <div class="card-body">
                                        ...
                                        <input type="hidden" name="uid" class="form-control" id="uid"
                                            ...
                                            <div class="row mb-3">
                                                    ...
                                                    <input type="text" id="oldname" name="oldname"  class="form-control" value="<?php echo $name; ?>" readonly>
                                                        <label for="mob">Associate Name</label>
                                            ...
                                            <div class="row mb-3">
                                                    ...
                                                    <span class="invalid-feedback"><?php echo $cou_name_err;?></span>
                                                        <label for="inputexc">Update Name</label>
